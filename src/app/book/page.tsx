      ...
      <section className="booking-panel">
        {!done ? <>
          ...
          {step === 1 && <div className="book-step"><div className="book-title"><span>Choose your device</span><h2>What are we repairing?</h2><p>Select a brand, then search or browse models.</p></div>
            <div className="brand-tabs">{brands.map((b) => <button className={brand === b ? "active" : ""} onClick={() => { setBrand(b); setDevice("") }} key={b}><img src={brandLogos[b]} alt="" /><span>{b}</span></button>)}</div>
            ...
            <div className="model-grid">{visibleDevices.map((d) => <button onClick={() => setDevice(d.model)} className={device === d.model ? "selected" : ""} key={d.model}>{d.badge && <em>{d.badge}</em>}<div className="model-phone-photo"><img src={getDeviceImage(d)} alt={`${d.model} phone`} /></div><b>{d.model}</b><span>{d.series}</span>{device === d.model && <i><Check /></i>}</button>)}</div>
          </div>}
